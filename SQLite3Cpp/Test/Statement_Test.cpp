 ...
 * @brief  Unit test file for Statement class
 * @version 0.1
 * @date 2022-11-25
 *
...
TEST( Statement, InValidTest ) {
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
  EXPECT_EQ( SQLite::OK, db.getExtendedErrorCode() );

  // Compile a SQL query, but without may table in the database there is no created table
  ...
  // Compile a SQLite query with no parameter
...
SQLite::Statement StatementBuilder( const SQLite::Database& aDb, const char* apQuery ) {
  return SQLite::Statement( aDb, apQuery );
}

TEST( Statement, moveConstructorTest ) {
...
TEST( Statement, executeStep ) {
  ...
  // Try again to insert a new row with the same PRIMARY KEY (with an alternative method): "UNIQUE constraint failed:
  // test.id"
  SQLite::Statement insert2( db, "INSERT INTO test VALUES (1, \"impossible\", 456, 0.456)" );
  EXPECT_THROW( insert2.exec(), SQLite::Exception );
}

TEST( Statement, tryExecuteStepTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
...
TEST( Statement, bindByNameTest ) {
  ...
  // Create a new table
  EXPECT_EQ(
      0, db.exec( "CREATE TABLE test (id INTEGER PRIMARY KEY, msg TEXT, int INTEGER, long INTEGER, double REAL)" ) );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
...
TEST( Statement, bindByNameStringTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
  ...
  // Create a new table
  EXPECT_EQ(
      0, db.exec( "CREATE TABLE test (id INTEGER PRIMARY KEY, msg TEXT, int INTEGER, double REAL, long INTEGER)" ) );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
...
TEST( Statement, isColumnNullTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
...
TEST( Statement, isColumnNullByNameTest ) {
  // Create a new database
...
TEST( Statement, getColumnByNameTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
  EXPECT_EQ( SQLite::OK, db.getErrorCode() );
  EXPECT_EQ( SQLite::OK, db.getExtendedErrorCode() );
...
TEST( Statement, getColumnDeclaredTypeTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
...
TEST( Statement, getColumnsTest ) {
  struct GetRowTestStruct {
      ...
      GetRowTestStruct( int _id, const std::string& _msg ) : id( _id ), msg( _msg ), integer( -1 ), real( 0.0 ) {}
  };
...
TEST( Statement, getBindParameterCountTest ) {
  // Create a new database
  SQLite::Database db( ":memory:", SQLite::OPEN_READWRITE | SQLite::OPEN_CREATE );
  EXPECT_EQ( 0, db.exec( "CREATE TABLE test (id INTEGER PRIMARY KEY, msg TEXT)" ) );
